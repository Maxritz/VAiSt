...
VAIST_API VaistStatus vaist_kv_create(size_t c,size_t w,VaistKVCache**o){VaistKVCache*k;if(!o||!c||!w)return VAIST_INVALID_ARGUMENT;k=(VaistKVCache*)calloc(1,sizeof(*k));if(!k)return VAIST_OUT_OF_MEMORY;k->data=(float*)calloc(c*w,sizeof(float));if(!k->data){free(k);return VAIST_OUT_OF_MEMORY;}k->cap=c;k->width=w;*o=k;return VAIST_OK;}
VAIST_API void vaist_kv_destroy(VaistKVCache*k){if(k){free(k->data);free(k);}}
VAIST_API VaistStatus vaist_kv_write(VaistKVCache*k,size_t p,const float*d){if(!k||!d||p>=k->cap)return VAIST_INVALID_ARGUMENT;memcpy(k->data+p*k->width,d,k->width*sizeof(float));if(p+1>k->len)k->len=p+1;return VAIST_OK;}
VAIST_API VaistStatus vaist_kv_read(const VaistKVCache*k,size_t p,float*o){if(!k||!o||p>=k->len)return VAIST_INVALID_ARGUMENT;memcpy(o,k->data+p*k->width,k->width*sizeof(float));return VAIST_OK;}
