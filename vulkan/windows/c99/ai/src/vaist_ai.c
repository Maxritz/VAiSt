...
VAIST_API VaistStatus vaist_vector_cosine(const float*a,const float*b,size_t n,float*o){float d,na,nb;size_t i;if(!a||!b||!o||!n)return VAIST_INVALID_ARGUMENT;d=na=nb=0;for(i=0;i<n;i++){d+=a[i]*b[i];na+=a[i]*a[i];nb+=b[i]*b[i];}if(na==0||nb==0)return VAIST_INVALID_ARGUMENT;*o=d/(sqrtf(na)*sqrtf(nb));return VAIST_OK;}
